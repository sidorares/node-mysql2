...
await describe('connectTimeout covers the full handshake', async () => {
  ...
  // Send a valid greeting, then leave the handshake pending.
  ...
  const connection = mysql.createConnection({
    ...
    port,
    user: 'test_user',
    // The timeout is triggered manually, so keep it from racing the test.
  ...
  it('keeps the timer armed after sending the HandshakeResponse', () => {
    // @ts-expect-error: internal access
    strict.notEqual(connection.connectTimeout, null);
  });

  // Exercise the timeout path without relying on wall-clock timing.
  // @ts-expect-error: internal access
  connection._handleTimeoutError();

  const err = await settled;

  it('reports ETIMEDOUT when the timeout fires mid-handshake', () => {
...
await describe('connectTimeout is cleared once the handshake completes', async () => {
  ...
  server.on('connection', (conn) => {
    conn.on('error', () => {
      // Ignore client disconnects during cleanup.
    });
  ...
  it('clears the timer when the handshake completes', () => {
    // @ts-expect-error: internal access
