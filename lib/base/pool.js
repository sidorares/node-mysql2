...
class BasePool extends EventEmitter {
  ...
  getConnection(cb) {
    const _getConnection = (cb) => {
      ...
      // Absolute deadline so re-serve paths (dead connection, resetOnRelease
      // failure) that re-enqueue the waiter keep the original budget instead
      // of re-arming a fresh full timeout.
      const deadline = cb._acquireDeadline || Date.now() + timeout;
      const queued = (err, connection) => {
        clearTimeout(queued._acquireTimeout);
        queued._acquireTimeout = null;
        cb(err, connection);
      };
      queued._acquireDeadline = deadline;
      queued._acquireTimeout = setTimeout(
        () => {
          spliceConnection(this._connectionQueue, queued);
          const err = new Error('getConnection timed out.');
      ...
      return this._connectionQueue.push(queued);
    };
  ...
  // Removing a waiter from the queue also disarms its acquireTimeout: the
  // timeout bounds the queue wait only, not a replacement connect attempt.
  _dequeueConnectionRequest() {
    const cb = this._connectionQueue.shift();
    if (cb && cb._acquireTimeout) {
      clearTimeout(cb._acquireTimeout);
      cb._acquireTimeout = null;
    }
    return cb;
  }
  ...
  releaseConnection(connection) {
    ...
    if (!connection._pool) {
      ...
      if (this._connectionQueue.length) {
        cb = this._dequeueConnectionRequest();
  ...
  [Symbol.dispose]() {
    ...
    while (this._connectionQueue.length > 0) {
      const queuedCallback = this._dequeueConnectionRequest();
