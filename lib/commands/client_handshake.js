...
class ClientHandshake extends Command {
  ...
  sendCredentials(connection) {
    ...
    const clientAuthMethod = this.canUseAuthMethodDirectly(
      serverAuthMethod,
      isSecureConnection
    )
      ? serverAuthMethod
  ...
  calculateSha256Token(password, scramble) {
    // Calculate SHA256-based auth token for caching_sha2_password
    // This is the same algorithm used in lib/auth_plugins/caching_sha2_password.js
    if (!password) {
