...
Connection.prototype.handlePacket = function(packet) {
  ...
  var connection = this;
  var done;
  // sequential deserialising code
  // handle all protocol errors here ( OOB etc )
  try {
    done = this._command.execute(packet, this);
    if (done) {
      this.sequenceId = 0;
    ...
    connection._protocolError = 'PROTOCOL_UNKNOWN_ERROR'
    var err = new Error('Connection lost: The server closed the connection.');
    err.fatal = true;
    err.code = connection._protocolError;
    connection.emit('error', err);
