...
Connection.prototype.raw = function raw(sql) {
...
Connection.prototype.beginTransaction = function(options,cb) {
  ...
  if (options.isolationLevel) {
    self.query("SET TRANSACTION ISOLATION LEVEL " + options.isolationLevel, function (err) {
      if (err) return cb(err);
      self.query(startTransactionSQL,cb);
    });
  } else {
    self.query(startTransactionSQL,cb);
  }
