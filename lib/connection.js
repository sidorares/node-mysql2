...
Connection.prototype._resolveNamedPlaceholders = function(options) {
  ...
  if (this.config.namedPlaceholders || options.namedPlaceholders) {
    ...
    if (this.config.logNamedPlaceholdersConversion || options.logNamedPlaceholdersConversion) {
      console.log('Converting named parameters. Original SQL: ', options.sql, 'Original parameters; ', options.values)
    }
    unnamed = convertNamedPlaceholders(options.sql, options.values);
    options.sql = unnamed[0];
    options.values = unnamed[1];
    if (this.config.logNamedPlaceholdersConversion || options.logNamedPlaceholdersConversion) {
      console.log('Converting named parameters done. New SQL: ', options.sql, 'New parameters; ', options.values)
    }
  }
};
