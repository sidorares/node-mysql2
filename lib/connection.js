...
function Connection(opts)
{
  ...
  this._inflatedPacketsParser = null;
  ...
  this.received = 0;
    this.stream.on('data', function(data) {
      connection.received += data.length;
...
Connection.prototype._enableCompression = function() {
  var self = this;
  this._inflatedPacketsParser = new PacketParser(function(p) {
    self.handlePacket(p);
  }, 4);
  this._inflatedPacketsParser._lastPacket = 0;
  this.packetParser = new PacketParser(function(packet) {
    self._handleCompressedPacket(packet);
  }, 7);
};
...
Connection.prototype.writePacket = function(packet) {
  ...
  //console.log('writePacket ====================', packetSequenceId);
...
Connection.prototype._checkOOOPackets = function() {
  var newOOO = [];
  var connection = this;
...
Connection.prototype._handleCompressedPacket = function(packet) {
  compressedPacketNum++;
  var connection = this;
