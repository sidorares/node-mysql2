...
class Connection extends EventEmitter {
  ...
  // given server hello packet, connection config and standard supported plugins
  // return the plugin to use for authentication
  getConnectAuthPlugin(pluginName) {
    // note that defaultAuthenticationPlugin semantics is "override the one sent by the server and use this instead for initial connection"
    // if defaultAuthenticationPlugin is not set, then use passed fallback plugin name, and if that is not set then use mysql_native_password
    const connectAuthPlugin = this.config.defaultAuthenticationPlugin ?? (pluginName ?? 'mysql_native_password');
    const userPlugins = this.config.authPlugins;
    if (userPlugins && userPlugins[connectAuthPlugin]) {
      return userPlugins[connectAuthPlugin];
    }

    // auth_plugins exports a map of plugin names to plugin "factory". We call it with default options.
    // if the user needs non-default options, they can use the authPlugins option to pass in a map of plugin names to an instance of the plugin
    const standardPlugins = require('./auth_plugins');
    if (standardPlugins[pluginName]) {
      return standardPlugins[pluginName]({});
    }
    return null;
  }
