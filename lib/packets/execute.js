...
function toParameter(value, encoding, timezone, parameterHints) {
  ...
  let unsigned = parameterHints.flags & FieldFlags.UNSIGNED;
  ...
  if (value !== null) {
    ...
    console.log(parameterHints.columnType, parameterHints.columnName)
    switch(parameterHints.columnType) {
      ...
      case Types.INT24: // in binary protocol int24 is encoded in 4 bytes int32
        value = value | 0;
        length = 4;
        writer = unsigned ? packet.writeInt32() : packet.writeSInt32();
  ...
  console.log({ value, type, length, writer })
...
class Execute {
  ...
  toPacket() {
    ...
    if (this.statement.parameters.length > 0 && this.parameters.length !== this.statement.parameters.length ) {
