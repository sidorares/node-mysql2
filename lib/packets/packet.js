...
Packet.prototype.isEOF = function() {
  // TODO: fix when buffer replaced with buffer
  return this.buffer[0] == 0xfe && this.buffer.length < 9;
};
...
Packet.prototype.readLengthCodedString = function() {
  ...
  //  return null;
  this.offset += len;
  var sb = this.buffer.parent;
  var offset = this.buffer.offset + this.offset;
  return sb.utf8Slice(offset - len, offset);
  // safe, but slow. TODO: benchmark numbers here
  //    return this.buffer.slice(this.offset - len, this.offset).toString('utf-8');
};
