...
// JSON.parse reviver source access (proposal-json-parse-with-source),
// available from Node.js 22 / V8 12.2. Detected once; without it
// parseJson falls back to plain JSON.parse, which rounds integers beyond
// Number.MAX_SAFE_INTEGER to the nearest double.
...
class Packet {
  ...
  // Parses a JSON column value (MySQL JSON, or MariaDB JSON identified via
  // extended metadata). With supportBigNumbers, integers that cannot be
  // accurately represented as JavaScript Numbers are returned as exact
  // String objects, mirroring the option's behaviour for BIGINT columns.
