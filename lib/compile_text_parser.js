...
function compile (fields, options, config) {
  ...
  if (options.rowsAsArray) {
    result('var result = new Array(' + fields.length + ')');
  }

  if (typeof options.typeCast === 'function') {
    result('var wrap = ' + wrap.toString());
  }
  ...
  if (options.nestTables === true) {
    ...
    for (i = 0; i < resultTablesArray.length; i++) {
      result('this[' + srcEscape(resultTablesArray[i]) + '] = {};');
    }
  }
  ...
  for (i = 0; i < fields.length; i++) {
    ...
    result('// ' + fieldName + ': ' + typeNames[fields[i].columnType]);
    ...
    if (typeof options.typeCast === 'function') {
      result(lvalue + ' = options.typeCast(wrap(fields[' + i + '], ' + srcEscape(typeNames[fields[i].columnType]) + ', packet, ' + encodingExpr + '), function() { return ' + readCode + ';})');
    } else if (options.typeCast === false) {
      result(lvalue + ' = packet.readLengthCodedBuffer();');
    } else {
      result(lvalue + ' = ' + readCode + ';');
    }
  }
  ...
  if (!config.debug) {
    ...
    console.log(cardinal.highlight(result.toString()) + '\n');
  }
  return vm.runInThisContext(result.toString());
}
