...
PacketParser.prototype._flushLargePacket = function _flushLargePacket () {
  ...
  while (end - start >= 3) {
    ...
    if (end - start >= this.length + this.packetHeaderLength) { // at least one full packet
      if (this.length < MAX_PACKET_LENGTH && this.largePacketParts.length === 0) {
        ...
        this.largePacketParts.push(chunk.slice(start + this.packetHeaderLength, start + MAX_PACKET_LENGTH));
