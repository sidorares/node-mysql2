...
PacketParser.prototype.execute = function(chunk) {
  ...
  while (start < end) {
    if (this.state == 'head0' && (end - start) > 3) {
      ...
      if (end - start >= remainingPayload) { // last chunk for payload
        ...
        this.sequenceId = this.headerBuff[3];
        this.headerLen = 0;
        if (this.length === 0) {
          this.onPacket(new Packet(this.sequenceId, new Buffer(0), 0, 0));
          this.state = 'head0';
          return;
        }
        this.state = 'payload';
