...
PacketParser.prototype._flushLargePacket4 = function _flushLargePacket () {
  ...
  // Do we need to remember start packet id? Just using 0 as id here
  // I think we need to completely remove ids from packet API and only check
  // packets order here in packet_parser
  this.onPacket(new Packet(0, body, 0, body.length));
};
...
PacketParser.prototype._flushLargePacket7 = function _flushLargePacket () {
  ...
  var body = Buffer.concat(this.largePacketParts);
  this.largePacketParts.length = 0;
  var p = new Packet(0, body, 0, body.length);
  this.onPacket(p);
};
  ...
  while (end - start >= 3) {
    ...
    if (end - start >= this.length + this.packetHeaderLength) { // at least one full packet
      if (this.length < MAX_PACKET_LENGTH && this.largePacketParts.length === 0) {
        ...
        this.largePacketParts.push(chunk.slice(start + this.packetHeaderLength, start + this.packetHeaderLength + MAX_PACKET_LENGTH));
...
PacketParser.prototype.executePayload = function executePayload (chunk) {
  ...
  if (end - start >= remainingPayload) { // last chunk for payload
    ...
    if (this.length < MAX_PACKET_LENGTH && this.largePacketParts.length === 0) {
      ...
      //var part = payload.slice(this.packetHeaderLength, this.packetHeaderLength + this.length);
      //if (part[0] == 188) {
      //  debugger;
      //}
