...
`TracingChannel` is available in Node.js 19.9+, 20+, and was backported to later Node.js 18.x releases. On older versions where `TracingChannel` is not available, tracing is silently disabled with zero overhead.
