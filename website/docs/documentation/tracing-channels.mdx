---
tags: [Tracing, Diagnostics, TracingChannel, APM, Observability, OpenTelemetry]
---
...
mysql2 provides built-in instrumentation via Node.js [`TracingChannel`](https://nodejs.org/api/diagnostics_channel.html#class-tracingchannel) from the `node:diagnostics_channel` module. This allows APM tools (OpenTelemetry, Datadog, Sentry, etc.) and custom instrumentation to subscribe to query, execute, connect, and pool lifecycle events without monkey-patching.
...
<History
  records={[
    {
      version: '4.20.0',
      changes: ['Added TracingChannel support for native APM instrumentation.'],
    },
  ]}
/>
