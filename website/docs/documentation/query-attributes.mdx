...
- **MySQL 8.0.25+** is required.
- The [`component_query_attributes`](https://dev.mysql.com/doc/refman/8.0/en/query-attributes.html) server component must be installed for the server to expose attributes via SQL functions.
- To install the component, run: `INSTALL COMPONENT "file://component_query_attributes"` as a privileged user.
:::
